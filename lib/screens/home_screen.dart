...
class DashboardContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    print('[DEBUG] DashboardContent.build: profile=${profile?.displayName}, isAuthenticated=${authProvider.isAuthenticated}');
              ...
              ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(999), // matches border-radius: 999px
                  ),
                  elevation: 0,
                ),
                child: const Text(
  ...
  Widget _buildDashboardMain(UserProfile? profile) {
    final stats = profile?.stats ?? {'total_events': 0, 'completed': 0, 'upcoming': 0};
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            ...
            if (nextEvent != null)
              TextButton(
                onPressed: () {
                  print('[ACTION] View next event tapped: ${nextEvent.event.id}');
        ...
        if (nextEvent != null)
          Container(
            decoration: BoxDecoration(
              color: primaryColor.withOpacity(0.12), // matches rgba(23, 127, 218, 0.12)
  ...
  Widget _buildHistorySection(UserProfile? profile) {
    ...
    final completed = profile?.history.where((h) => h.status == 'completed').toList() ?? [];
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            Expanded(
              child: _buildHistoryList('Upcoming & Registered', upcoming, false),
            ),
            const SizedBox(width: 24), // matches gap: 1.5rem
            Expanded(
              child: _buildHistoryList('Completed', completed, true),
            ),
          ],
        ),
      ],
  ...
  Widget _buildHistoryList(String title, List<UserRaceHistory> history, bool isCompleted) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
        ...
        if (history.isEmpty)
          ...
          Column(
            children: history.map((entry) => Container(
              margin: const EdgeInsets.only(bottom: 8),
              padding: const EdgeInsets.all(16), // matches padding: 1rem
              decoration: BoxDecoration(
                color: primaryColor.withOpacity(0.08), // matches rgba(23, 127, 218, 0.08)
                borderRadius: BorderRadius.circular(12), // matches border-radius: 12px
              ),
              child: Row(
                children: [
                  Expanded(
                  ...
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), // matches padding: 0.25rem 0.65rem
                    decoration: BoxDecoration(
                      color: isCompleted ? accentColor.withOpacity(0.45) : primaryColor.withOpacity(0.2),
                      borderRadius: BorderRadius.circular(999), // matches border-radius: 999px
                    ),
                    child: Text(
                      ...
                      style: TextStyle(
                        ...
                        color: isCompleted ? const Color(0xFF14532D) : primaryColor, // matches color: #14532d for completed
                      ),
                    ),
                  ),
                ],
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (achievement.link != null) ...[
                    ...
                    TextButton(
                      onPressed: () {
                        print('[ACTION] Achievement link tapped: ${achievement.link}');
                        // TODO: Open link
                      },
                      style: TextButton.styleFrom(
